...
- [x] 1. Discover and inventory entity/schema source material
  ...
  - [x] 1.2 Inventory legacy SQL source material
    ...
    - Enumerate the PostgreSQL `bigdata/sql/ddl/`, `dml/`, `migration/` (V1â€“V13) tables to identify any tables absent from `init_mysql.sql`, including `login_logs` and `purchase_requests`
...
- [x] 2. Create the classpath Flyway schema migration
  - [x] 2.1 Create the classpath migration directory and V1__init_schema.sql
    ...
    - Apply the PostgreSQLâ†’MySQL 8.0 conversion rules; do not emit `CREATE EXTENSION`, `uuid_generate_v4()`, `TIMESTAMPTZ`, `JSONB`, `NUMERIC(...)`, `ON CONFLICT`, or `CREATE INDEX IF NOT EXISTS`
  ...
  - [x] 2.4 Create V2__seed_data.sql
    ...
    - Map legacy `data_scopes.scope_type` values `all` â†?`all_company` and `assigned` â†?`assigned_store`
...
- [x] 3. Normalize entity columnDefinition literals to MySQL types
  - [x] 3.1 Edit entity columnDefinition values from PostgreSQL to MySQL literals
    - Using the list from task 1.1, change `columnDefinition = "uuid"` â†?`"char(36)"` and `columnDefinition = "jsonb"` â†?`"json"` across all affected entities (e.g. `User`, `DataScope` `storeIds`/`productIds`, `WarehouseInventoryEntity`, `ProductUploadJobEntity` `payload`/`response`, and others discovered)
