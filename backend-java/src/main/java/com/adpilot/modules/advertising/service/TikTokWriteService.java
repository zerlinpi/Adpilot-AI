...
/**
 * TikTok Shop inventory / fulfillment write-back orchestration (Req 4, TikTok family).
 *
 * <p>Backs {@code POST /api/tiktok/products/{productId}/inventory} and
 * {@code POST /api/tiktok/orders/{orderId}/fulfillment}. It is the exact TikTok
 * analogue of {@link IndependentSiteWriteService}: every write is gated and, when
 * allowed, routed through the Operation-Outbox so the external TikTok Shop platform
 * is never called on the request thread (Req 4.2). The only differences from the
 * independent-site service are the platform family ({@code tiktok}) and the target
 * platform key ({@code tiktok_shop}).
 *
 * <h2>Connection gating (Req 4.1, 4.3)</h2>
 * A write is allowed only when the target store has a state-{@code connected}
 * {@code tiktok_shop} connection AND that platform is actually write-capable (a
 ...
 * platform call. The {@code OutboxWorker} discovers the {@code tiktok_shop}
 * connector and submits the change asynchronously.
 *
 ...
 */
public interface TikTokWriteService {

    /**
     * Enqueue an inventory update for a TikTok Shop product (Req 4.1, 4.2).
     *
     * @param productId the TikTok Shop product whose inventory is set (path id)
    ...
    /**
     * Enqueue a fulfillment / shipment mark for a TikTok Shop order (Req 4.1, 4.2).
     *
     * @param orderId the TikTok Shop order being marked fulfilled (path id)
     ...
     * TikTok Shop stores (Req 4.4, 4.7).
     *
     * <p>For each in-scope {@code tiktok_shop}
