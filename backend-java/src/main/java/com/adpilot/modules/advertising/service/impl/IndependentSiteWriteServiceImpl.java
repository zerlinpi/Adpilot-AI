...
/**
 * Default {@link IndependentSiteWriteService}.
 *
 * <p>Implements the connection-gated, Outbox-asynchronous write-back contract for
 * independent-site inventory updates and fulfillment marks (Req 4.1, 4.2, 4.3,
 * 4.5). The flow is identical for both write kinds and is captured by
 * {@link #enqueueWrite}: gate the store on its Shopify / WooCommerce connection and
 ...
 *       Shopify / WooCommerce connection (no valid connection / missing
 *       credentials). Nothing is persisted and no write-back is recorded.</li>
...
@Slf4j
@Service
public class IndependentSiteWriteServiceImpl implements IndependentSiteWriteService {

    /** Independent-site platforms that support inventory / fulfillment write-back. */
    static final List<String> INDEPENDENT_SITE_PLATFORMS = List.of("shopify", "woocommerce");
    ...
    /** Operation entity types and fields routed to the independent-site write connectors. */
    ...
    /** Independent-site write-back is confined to the {@code independent_site} platform family (Req 4.6). */
    private static final PlatformFamily REQUIRED_FAMILY = PlatformFamily.INDEPENDENT_SITE;
    ...
    public IndependentSiteWriteServiceImpl(OperationService operationService,
    ...
    public List<IndependentSiteConnectionStateVo> getConnectionStates(String storeId) {
        ...
        wrapper.in("platform", INDEPENDENT_SITE_PLATFORMS);
     ...
     * independent-site write (Req 4.6). The caller must be able to write the target
     * store within their data scope, and the target store's {@code platform_family}
     * must be {@code independent_site}; a store outside scope or of another family is
     * rejected with HTTP 403 before any Operation/Outbox is created.
     */
    private void assertWriteScopeAndFamily(UUID storeId) {
        ...
        // Platform-family isolation: the target store must belong to the independent_site family.
        StoreEntity store = storeId != null ? storeMapper.selectById(storeId) : null;
        if (store == null || !isIndependentSiteFamily(store.getPlatformFamily())) {
            log.info("Independent-site write refused (cross-family): store={} family={} expected={}",
                    storeId, store != null ? store.getPlatformFamily() : null, REQUIRED_FAMILY.getCode());
            throw forbiddenFamily(
                    "Store " + storeId + " is not an independent-site store; "
                            + "independent-site write-back is confined to the independent_site platform family");
        }
    }

    private static boolean isIndependentSiteFamily(String platformFamily) {
     ...
     * Shopify / WooCommerce connection (Req 4.3) and with HTTP 409 when a
     * connection exists but the platform is not yet write-capable (Req 4.7). In
     * both refusal cases nothing is persisted — {@link OperationService#createOperation}
     * is never reached, so no Operation or Outbox entry is created (Req 4.3).
     */
    private OperationActionVo enqueueWrite(UUID storeId, String entityType, UUID entityId, String field,
        ...
        // Req 4.3 — refuse when the store has no valid, connected independent-site connection.
        boolean hasConnectedSite = hasConnectedIndependentSiteConnection(storeId);
        if (!hasConnectedSite) {
            log.info("Independent-site write refused (not authorized): store={} action={} — no connected "
                    + "Shopify/WooCommerce connection", storeId, action);
            throw notAuthorized(
                    "Store " + storeId + " has no connected Shopify/WooCommerce connection; "
        ...
        if (!writeCapabilityService.isWriteCapable(storeId)) {
            log.info("Independent-site write refused (unsupported): store={} action={} — write capability "
        ...
        log.info("Independent-site {} Operation created: store={} entity={}/{} operation={} syncState={}",
    ...
    /**
     * Whether the store has at least one state-{@code connected} Shopify / WooCommerce
     * connection — the "valid connection / has credentials" precondition of Req 4.1 / 4.3.
     */
    private boolean hasConnectedIndependentSiteConnection(UUID storeId) {
        if (storeId == null) {
            return false;
        }
        List<PlatformConnectionEntity> connections = platformConnectionMapper.selectList(
                new LambdaQueryWrapper<PlatformConnectionEntity>()
                        ...
                        .in(PlatformConnectionEntity::getPlatform, INDEPENDENT_SITE_PLATFORMS));
        return connections.stream().anyMatch(IndependentSiteWriteServiceImpl::isConnected);
    }
    ...
    private static BusinessException badRequest(String message) {
        return new BusinessException(400, "INVALID_INDEPENDENT_SITE_WRITE", message);
    }
    ...
    private static BusinessException notAuthorized(String message) {
        return new BusinessException(403, "INDEPENDENT_SITE_NOT_AUTHORIZED", message);
    }
    ...
    private static BusinessException unsupported(String message) {
        return new BusinessException(409, "INDEPENDENT_SITE_WRITE_UNSUPPORTED", message);
    }
    ...
    private static BusinessException forbiddenFamily(String message) {
        return new BusinessException(403, "INDEPENDENT_SITE_CROSS_FAMILY", message);
    }
}
